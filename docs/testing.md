...
| `test_cases.py` | scene claims | Sod density + velocity errors and convergence; Taylor–Green analytical decay; diffusion-only dye vs exp(−2κm²t); conduction baseline Nu = 1; porous k_x ≫ k_y, Darcy linearity, disconnected sample; SPH still water; airfoil lift sign mirror; registry completeness; tracer: pure diffusion variance = 2·D_m·t, mass conserved with grains and none inside them, dispersion above molecular and above the scheme's own numerical diffusion, open outlet (a pulse leaves and the inlet end runs clean again, a steady supply settles at the injected concentration without exceeding it, and the balance closes), shipped scene runs and measures; every scene inside its solver's limits. Checks added by the numerical audit: a reversed outlet face draws clean water rather than the feed; the transport step stays monotone when advection and diffusion act together, with nothing clipped away; plume moments weigh tracer rather than pore-averaged concentration; an unstable wind tunnel stops instead of returning non-finite frames as a completed run; water frames hold one state per time; the Gray-Scott frame at time zero is the state before any update; the tracer reports when the flow it froze was neither settled nor creeping, and how much of a pore-volume crossing the step budget covered; spectral products do not alias into the retained band; held blocks neither gain nor lose mass; reported extrema cover every step and the final state; the speed limiter reports what it suppressed; tracer and mixing runs keep the state they finished in; and a setting measured to go unstable warns before it starts. 31 checks in all |
